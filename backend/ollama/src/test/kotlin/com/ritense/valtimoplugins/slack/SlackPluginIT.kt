...
package com.ritense.valtimoplugins.ollama
...
@Transactional
class OllamaPluginIT : BaseIntegrationTest() {
    ...
    internal fun setUp() {
        startMockServer()
        configuration = createOllamaPluginConfiguration()
    }
    ...
    private fun createOllamaPluginConfiguration(): PluginConfiguration {
        val ollamaConfigurationProperties = """
            {
                "url": "${server.url("/")}",
                "token": "test-token"
            }"""
        ...
        return pluginService.createPluginConfiguration(
            ...
            MapperSingleton.get().readTree(ollamaConfigurationProperties) as ObjectNode,
            "ollama"
        )
    }
