...
@Transactional
class DocsysPluginIT : BaseIntegrationTest() {
    ...
    internal fun setUp() {
        startMockServer()
        configuration = createDocsysPluginConfiguration()
    }
...
//        }
 ...
 //       server.dispatcher = dispatcher
    ...
    private fun createDocsysPluginConfiguration(): PluginConfiguration {
        val docsysConfigurationProperties = """
            {
                "url": "${server.url("/")}",
                "token": "test-token"
            }"""
        ...
        return pluginService.createPluginConfiguration(
            "Docsys plugin configuration",
            MapperSingleton.get().readTree(docsysConfigurationProperties) as ObjectNode,
            "docsys"
        )
    }
