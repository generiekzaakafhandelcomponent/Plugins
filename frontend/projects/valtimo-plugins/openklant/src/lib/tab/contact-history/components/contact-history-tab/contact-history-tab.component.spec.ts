...
import {mockCustomerContactDTO, mockProcessInstance, mockProcessInstanceStartResponse} from '../models/mocks';
...
describe('ContactHistoryTabComponent', () => {
    ...
    function setupTestBed(routeParamMap: Map<string, string> = new Map([['documentId', 'mock-document-id']])) {
        TestBed.configureTestingModule({
            ...
            providers: [
                ...
                },
                {
                    ...
                    useValue: {
                        load: jasmine.createSpy('load').and.returnValue(of([mapDtoToModel(mockCustomerContactDTO)]))
                    }
                },
                ProcessPollingService,
                {
                    ...
                    useValue: {
                        createExponentialPollingTicks: jasmine.createSpy().and.returnValue(of(0, 1, 2))
                    }
                },
                {
                    provide: NGXLogger,
    ...
    it('should be created as an instance of ContactHistoryTabComponent', () => {
        ...
        expect(fixture.componentInstance).toBeInstanceOf(ContactHistoryTabComponent);
    });

    it('should load the document and map the contact history', () => {
        const {component} = setupTestBed();
        const loadContactHistorySpy = TestBed.inject(ContactHistoryService).load as jasmine.Spy;
    ...
    it('should do nothing and go into isFailed state if no documentId is available in the route snapshot paramMap', () => {
        ...
        const loadContactHistorySpy = TestBed.inject(ContactHistoryService).load as jasmine.Spy;
        const startProcessInstanceSpy = TestBed.inject(ProcessService).startProcesInstance as jasmine.Spy;
    ...
    it(`should retrieve the document's contactHistory twice (at init and after finishing the called process)`, fakeAsync(() => {
        ...
        startProcessInstanceSpy.and.returnValue(of({...mockProcessInstanceStartResponse, ended: true}));
    ...
    it('should stop polling as soon as the processService.getProcessInstance response has the property .endtime', fakeAsync(() => {
        const {component} = setupTestBed();
        const pollingTicksSpy = TestBed.inject(PollingService).createExponentialPollingTicks as jasmine.Spy;
        const startProcessInstanceSpy = TestBed.inject(ProcessService).startProcesInstance as jasmine.Spy;
        const getProcessInstanceSpy = TestBed.inject(ProcessService).getProcessInstance as jasmine.Spy;
        startProcessInstanceSpy.and.returnValue(of(mockProcessInstanceStartResponse));
    ...
    it('should start polling if the response of the call to startProcesInstance has the property ended: false', fakeAsync(() => {
        ...
        const pollUntilStoppedSpy = spyOn(processPollingService, 'pollUntilStopped').and.returnValue(
            of({...mockProcessInstance, endTime: new Date().toISOString()}) // first poll completes stream since it has an endTime
        );
        startProcessInstanceSpy.and.returnValue(of({...mockProcessInstanceStartResponse, ended: false}));
