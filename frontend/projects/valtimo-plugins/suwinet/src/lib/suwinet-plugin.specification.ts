...
const suwinetPluginSpecification: PluginSpecification = {
    ...
    pluginTranslations: {
        nl: {
            ...
            periodeFilterInfo: 'Ondersteunde notaties:\n• ISO-datum (2024-01-01)\n• Valtimo value resolver bijv. (pv:aanvraagdatum)\n• SpEL-expressie bijv. (${localDateTimeNow.minusWeeks(2)})\n• SpEL met procesvariabele: (${#aanvraagdatum.minusMonths(1)}) — variabelen in SpEL beginnen met #\n\nStartdatum leeg = geen filtering.',
            startdatumPeriode: 'Startdatum periode',
            startdatumPeriodeTooltip: 'Expressie voor de startdatum van de filterperiode. Gebruik een ISO-datum (2024-01-01), alle Valtimo value resolvers (zoals pv:, doc:, case:) of een SpEL-expressie zoals ${localDateTimeNow.minusWeeks(2)}. Leeg = geen filtering.',
            einddatumPeriode: 'Einddatum periode',
            einddatumPeriodeTooltip: 'Expressie voor de einddatum van de filterperiode. Ondersteunt dezelfde notaties als startdatum. Leeg = huidige datum (als startdatum is opgegeven).',
        ...
        en: {
            ...
            periodeFilterInfo: 'Supported notations:\n• ISO date (2024-01-01)\n• Valtimo value resolver e.g. (pv:aanvraagdatum)\n• SpEL expression e.g. (${localDateTimeNow.minusWeeks(2)})\n• SpEL with process variable: (${#aanvraagdatum.minusMonths(1)}) — variables inside SpEL must start with #\n\nStart date empty = no filtering.',
            startdatumPeriode: 'Period start date',
            startdatumPeriodeTooltip: 'Expression for the period filter start date. Use an ISO date (2024-01-01), any Valtimo value resolver (such as pv:, doc:, case:) or a SpEL expression like ${localDateTimeNow.minusWeeks(2)}. Empty = no filtering.',
            einddatumPeriode: 'Period end date',
            einddatumPeriodeTooltip: 'Expression for the period filter end date. Supports the same notations as start date. Empty = current date (if start date is set).',
            connectionTimeout: 'connectionTimeout: Specifies the amount of time, in seconds, that the consumer will attempt to establish a connection before it times out. 0 is infinite',
            receiveTimeout: 'receiveTimeout: Specifies the amount of time, in seconds, that the consumer will wait for a response before it times out. 0 is infinite.',
        ...
        de: {
            ...
            maxPeriods: 'Maximale Perioden',
            periodeFilterInfo: 'Unterstützte Notationen:\n• ISO-Datum (2024-01-01)\n• Valtimo Value Resolver z.B. (pv:aanvraagdatum)\n• SpEL-Ausdruck z.B. (${localDateTimeNow.minusWeeks(2)})\n• SpEL mit Prozessvariable: (${#aanvraagdatum.minusMonths(1)}) — Variablen in SpEL beginnen mit #\n\nStartdatum leer = keine Filterung.',
            startdatumPeriode: 'Periodenstartzeitpunkt',
            startdatumPeriodeTooltip: 'Ausdruck für das Startdatum des Filterzeitraums. Verwenden Sie ein ISO-Datum (2024-01-01), einen beliebigen Valtimo Value Resolver (z. B. pv:, doc:, case:) oder einen SpEL-Ausdruck wie ${localDateTimeNow.minusWeeks(2)}. Leer = keine Filterung.',
            einddatumPeriode: 'Periodenendezeitpunkt',
            einddatumPeriodeTooltip: 'Ausdruck für das Enddatum des Filterzeitraums. Unterstützt dieselben Notationen wie das Startdatum. Leer = aktuelles Datum (wenn Startdatum angegeben).',
            connectionTimeout: 'connectionTimeout: Specifies the amount of time, in seconds, that the consumer will attempt to establish a connection before it times out. 0 is infinite',
            receiveTimeout: 'receiveTimeout: Specifies the amount of time, in seconds, that the consumer will wait for a response before it times out. 0 is infinite.',
